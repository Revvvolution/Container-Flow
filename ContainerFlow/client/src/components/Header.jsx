...
export default function Header() {
  ...
  return (
    ...
    <header className="flex flex-row items-center justify-between border-4 border-yellow-800/40 w-[100vw] h-full px-1 mt-0.2 bg-gradient-to-b from-cyan-600/75 via-cyan-700/90 to-zinc-500/90 md:h-20 md:bg-cyan-600 sm:justify-around col-span-2 p-2 border-b-2 z-10">
            <Link
                to={"/"}
                className="hidden absolute left-4 top-4 md:top-6 md:flex items-center h-10 px-10 bg-gradient-to-r from-gray-900 via-gray-600 to-gray-500 rounded-tl-full rounded-br-full font-bold uppercase italic text-white hover:opacity-90 hover:shadow-[0_10px_30px_5px_rgba(255,255,255,0.2)]"
            >
                ContainerFlow
            </Link>
      ...
      <nav className="sm:hidden flex flex-col items-end gap-1 mt-40 w-full font-semibold bg-gradient-to-b from-cyan-600/75 via-cyan-700/90 to-zinc-500/90 z-30">
        ...
        >
          {showMenu ? <GrClose className="mt-2 absolute right-6 top-1.5 size-8" /> : <GiHamburgerMenu className="absolute right-6 top-1.5 size-8"/>}
        </button>
        {showMenu && (
          <>
